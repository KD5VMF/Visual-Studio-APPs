...
public sealed class Brain
{
    ...
    public static Brain Breed(Brain a, Brain b, Random rng, float sigma)
    {
        if (a.InputCount != b.InputCount || a.HiddenCount != b.HiddenCount || a.OutputCount != b.OutputCount)
        {
            return a.MutatedClone(rng, sigma);
        }

        var w1 = new float[a._w1.Length];
        var b1 = new float[a._b1.Length];
        var w2 = new float[a._w2.Length];
        var b2 = new float[a._b2.Length];
        var adaptive = new float[a._adaptiveBias.Length];

        for (var i = 0; i < w1.Length; i++) w1[i] = (((i & 1) == 0 ? a._w1[i] : b._w1[i]) + ((a._w1[i] + b._w1[i]) * 0.5f)) * 0.5f + (Gaussian(rng) * sigma * 0.35f);
        for (var i = 0; i < w2.Length; i++) w2[i] = (((i & 1) == 0 ? a._w2[i] : b._w2[i]) + ((a._w2[i] + b._w2[i]) * 0.5f)) * 0.5f + (Gaussian(rng) * sigma * 0.35f);
        for (var i = 0; i < b1.Length; i++) b1[i] = ((a._b1[i] + b._b1[i]) * 0.5f) + (Gaussian(rng) * sigma * 0.20f);
        for (var i = 0; i < b2.Length; i++) b2[i] = ((a._b2[i] + b._b2[i]) * 0.5f) + (Gaussian(rng) * sigma * 0.20f);
        for (var i = 0; i < adaptive.Length; i++) adaptive[i] = ((a._adaptiveBias[i] + b._adaptiveBias[i]) * 0.35f);

        return new Brain(a.InputCount, a.HiddenCount, a.OutputCount, w1, b1, w2, b2, adaptive, MathF.Max(a.Plasticity, b.Plasticity));
    }
    ...
    public BrainState CaptureState()
    {
        return new BrainState
        {
            ...
            W2 = (float[])_w2.Clone(),
            B2 = (float[])_b2.Clone(),
            AdaptiveBias = (float[])_adaptiveBias.Clone()
        };
    }

    public static Brain FromState(BrainState? state, Random rng)
    {
        if (state is null ||
            state.InputCount <= 0 ||
            state.HiddenCount <= 0 ||
            state.OutputCount <= 0 ||
            state.W1.Length != state.HiddenCount * state.InputCount ||
            state.B1.Length != state.HiddenCount ||
            state.W2.Length != state.OutputCount * state.HiddenCount ||
            state.B2.Length != state.OutputCount ||
            state.AdaptiveBias.Length != state.OutputCount)
        {
            return new Brain(22, 24, 8, rng);
        }
        ...
        return new Brain(
            ...
            state.HiddenCount,
            state.OutputCount,
            (float[])state.W1.Clone(),
            (float[])state.B1.Clone(),
            (float[])state.W2.Clone(),
            (float[])state.B2.Clone(),
            (float[])state.AdaptiveBias.Clone(),
            Math.Clamp(state.Plasticity, 0.01f, 1.00f));
    }
...
public sealed class Vec2State
{
    public float X { get; set; }
    public float Y { get; set; }

    public Vec2 ToVec2() => new(X, Y);
    public static Vec2State FromVec2(in Vec2 value) => new() { X = value.X, Y = value.Y };
}
...
public sealed class BrainState
{
    ...
    public float[] W1 { get; set; } = Array.Empty<float>();
    public float[] B1 { get; set; } = Array.Empty<float>();
    public float[] W2 { get; set; } = Array.Empty<float>();
    public float[] B2 { get; set; } = Array.Empty<float>();
    public float[] AdaptiveBias { get; set; } = Array.Empty<float>();
}

public sealed class CultureMemoryState
{
    public Vec2State FoodDirection { get; set; } = new();
    public Vec2State DangerDirection { get; set; } = new();
    public Vec2State WaterDirection { get; set; } = new();
    public Vec2State HuntDirection { get; set; } = new();
    public float FoodStrength { get; set; }
    public float DangerStrength { get; set; }
    public float WaterStrength { get; set; }
    public float HuntStrength { get; set; }
...
public sealed class AgentSeedState
{
    public int Generation { get; set; }
    public Sex Sex { get; set; }
    public AgentTraits Traits { get; set; } = new();
    public BrainState Brain { get; set; } = new();
    public float CooperationBias { get; set; }
    public float CuriosityBias { get; set; }
    public float CourageBias { get; set; }
    public float AggressionBias { get; set; }
    public Vec2State FoodMemory { get; set; } = new();
    public Vec2State DangerMemory { get; set; } = new();
    public Vec2State WaterMemory { get; set; } = new();
    public Vec2State HuntMemory { get; set; } = new();
    public float FoodMemoryStrength { get; set; }
    public float DangerMemoryStrength { get; set; }
    public float WaterMemoryStrength { get; set; }
    public float HuntMemoryStrength { get; set; }
    public int ReinvestmentStreak { get; set; }
    public int PeakReinvestmentStreak { get; set; }
    public int DeferredOffspring { get; set; }
    public int VisionTrades { get; set; }
    public int SpeedTrades { get; set; }
    public int AccelerationTrades { get; set; }
    public int EfficiencyTrades { get; set; }
    public int AwarenessTrades { get; set; }
}

public sealed class TrainingBankState
{
    public int Version { get; set; } = 1;
    public int Slot { get; set; } = -1;
    public long SaveIndex { get; set; }
    public int RebirthCount { get; set; }
    public string Trigger { get; set; } = "fresh-start";
    public DateTime SavedUtc { get; set; } = DateTime.UtcNow;
    public CultureMemoryState PreyCulture { get; set; } = new();
    public CultureMemoryState PredatorCulture { get; set; } = new();
    public List<AgentSeedState> PreySeeds { get; set; } = new();
    public List<AgentSeedState> PredatorSeeds { get; set; } = new();
}

public sealed class SpeciesTrainingState
{
    public int Version { get; set; } = 1;
    ...
    public string Trigger { get; set; } = "fresh-start";
    public DateTime SavedUtc { get; set; } = DateTime.UtcNow;
    public CultureMemoryState Culture { get; set; } = new();
    public List<AgentSeedState> Seeds { get; set; } = new();
}

public sealed class TrainingPersistence
{
    ...
    public TrainingPersistence(string rootDirectory)
    {
    ...
    public string PreyPath => Path.Combine(_rootDirectory, "Prey_Training.json");
    public string PredatorPath => Path.Combine(_rootDirectory, "Predator_Training.json");

    public bool TryLoadLatest(out TrainingBankState state)
    {
        state = new TrainingBankState();

        var loadedAny = false;

        var preyState = LoadSpeciesFile(PreyPath);
        if (preyState is not null)
        {
            state.PreySeeds = preyState.Seeds ?? new List<AgentSeedState>();
            state.PreyCulture = preyState.Culture ?? new CultureMemoryState();
            state.SaveIndex = Math.Max(state.SaveIndex, preyState.SaveIndex);
            state.RebirthCount = Math.Max(state.RebirthCount, preyState.RebirthCount);
            if (preyState.SavedUtc >= state.SavedUtc)
            {
                state.SavedUtc = preyState.SavedUtc;
                state.Trigger = preyState.Trigger ?? state.Trigger;
            }
            loadedAny = true;
        }

        var predatorState = LoadSpeciesFile(PredatorPath);
        if (predatorState is not null)
        {
            state.PredatorSeeds = predatorState.Seeds ?? new List<AgentSeedState>();
            state.PredatorCulture = predatorState.Culture ?? new CultureMemoryState();
            state.SaveIndex = Math.Max(state.SaveIndex, predatorState.SaveIndex);
            state.RebirthCount = Math.Max(state.RebirthCount, predatorState.RebirthCount);
            if (predatorState.SavedUtc >= state.SavedUtc)
            {
                state.SavedUtc = predatorState.SavedUtc;
                state.Trigger = predatorState.Trigger ?? state.Trigger;
            }
            loadedAny = true;
        }
    ...
    public void Save(TrainingBankState state)
    {
        Directory.CreateDirectory(_rootDirectory);

        SaveSpeciesFile(
            PreyPath,
            new SpeciesTrainingState
            {
                SpeciesName = "Prey",
                SaveIndex = state.SaveIndex,
                RebirthCount = state.RebirthCount,
                Trigger = state.Trigger,
                SavedUtc = state.SavedUtc,
                Culture = state.PreyCulture ?? new CultureMemoryState(),
                Seeds = state.PreySeeds ?? new List<AgentSeedState>()
            });
        ...
        SaveSpeciesFile(
            ...
            new SpeciesTrainingState
            {
                ...
                Trigger = state.Trigger,
                SavedUtc = state.SavedUtc,
                Culture = state.PredatorCulture ?? new CultureMemoryState(),
                Seeds = state.PredatorSeeds ?? new List<AgentSeedState>()
            });
    }

    private SpeciesTrainingState? LoadSpeciesFile(string path)
    {
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<SpeciesTrainingState>(text, _jsonOptions);
        }
        catch
        {
            return null;
    ...
    private void SaveSpeciesFile(string path, SpeciesTrainingState state)
    {
...
public sealed class CultureMemory
{
    ...
    public void Learn(KnowledgeKind kind, Vec2 direction, float strength, AgentTraits traits)
    {
        ...
        var aggressionSource = kind == KnowledgeKind.Hunt ? (traits.Courage + 0.30f) : (traits.Courage * 0.18f);
        Aggression = Math.Clamp((Aggression * 0.92f) + (aggressionSource * 0.08f), 0f, 1f);
        Vigilance = Math.Clamp((Vigilance * 0.90f) + ((traits.Courage + (kind == KnowledgeKind.Danger ? 0.36f : 0f)) * 0.12f), 0f, 1f);
    }


    public CultureMemoryState CaptureState()
    {
        return new CultureMemoryState
        {
            FoodDirection = Vec2State.FromVec2(FoodDirection),
            DangerDirection = Vec2State.FromVec2(DangerDirection),
            WaterDirection = Vec2State.FromVec2(WaterDirection),
            HuntDirection = Vec2State.FromVec2(HuntDirection),
            FoodStrength = FoodStrength,
            DangerStrength = DangerStrength,
            WaterStrength = WaterStrength,
            HuntStrength = HuntStrength,
            Cooperation = Cooperation,
    ...
    public void ApplyState(CultureMemoryState? state)
    {
        Reset();
        if (state is null) return;

        FoodDirection = state.FoodDirection?.ToVec2() ?? Vec2.Zero;
        DangerDirection = state.DangerDirection?.ToVec2() ?? Vec2.Zero;
        WaterDirection = state.WaterDirection?.ToVec2() ?? Vec2.Zero;
        HuntDirection = state.HuntDirection?.ToVec2() ?? Vec2.Zero;
        FoodStrength = Math.Clamp(state.FoodStrength, 0f, 1f);
        DangerStrength = Math.Clamp(state.DangerStrength, 0f, 1f);
        WaterStrength = Math.Clamp(state.WaterStrength, 0f, 1f);
        HuntStrength = Math.Clamp(state.HuntStrength, 0f, 1f);
        Cooperation = Math.Clamp(state.Cooperation, 0f, 1f);
        Aggression = Math.Clamp(state.Aggression, 0f, 1f);
        Vigilance = Math.Clamp(state.Vigilance, 0f, 1f);
        Broadcasts = Math.Max(0, state.Broadcasts);
    }
...
public sealed class Agent
{
    ...
    public int ReinvestmentStreak;
    public int PeakReinvestmentStreak;
    public int DeferredOffspring;
    public int VisionTrades;
    public int SpeedTrades;
    public int AccelerationTrades;
    public int EfficiencyTrades;
    public int AwarenessTrades;
    public float HiddenDeathAge;
    public Sex Sex;
    public bool IsElder;
    public float HideAffinity;
    public int MateTargetId = -1;
    ...
    public void Integrate(World world, Vec2 steering, float sprint, float dt)
    {
        ...
        if (Energy <= 0f || Hydration <= 0f || Age >= HiddenDeathAge)
        {
            IsDead = true;
        }
    }
    ...
    {
        yield return $"{Species} {Sex} #{Id}  gen {Generation}";
        ...
        yield return $"Invest {ReinvestmentStreak,6}   Peak    {PeakReinvestmentStreak,6}";
        yield return $"Stored babies {DeferredOffspring,3}   Hide {HideAffinity,5:F2}";
        yield return $"Mate target {(MateTargetId > 0 ? MateTargetId : 0),4}";
        yield return $"Legacy end {HiddenDeathAge,6:F1}   Life left {Math.Max(0f, HiddenDeathAge - Age),6:F1}";
        yield return $"Trades V:{VisionTrades} S:{SpeedTrades} A:{AccelerationTrades} E:{EfficiencyTrades} W:{AwarenessTrades}";
...
public sealed class World
{
    ...
    private const float AutosaveIntervalSeconds = 600f;
    private const int PreySeedPoolSize = 48;
    private const int PredatorSeedPoolSize = 24;
    private const int PreyPopulationCount = 180;
    private const int PredatorPopulationCount = 20;
    ...
    private readonly TrainingPersistence _trainingPersistence;
    private TrainingBankState _trainingBank = new();
    ...
    private bool _preyRecoveryPending;
    private bool _trainingDirty;
    private string _lastPersistenceEvent = "fresh-start";
    ...
    public Agent? OldestLivingPrey { get; private set; }
    public Agent? OldestLivingPredator { get; private set; }
    ...
    public long CurrentTrainingSaveIndex => _trainingBank.SaveIndex;
    public int RebirthCount => _trainingBank.RebirthCount;
    public string TrainingSlotLabel => "prey/pred";
    public string LastPersistenceEvent => _lastPersistenceEvent;
    public string TrainingSaveDirectory => _trainingPersistence.RootDirectory;
    public Vec2 WorldCenter => new(Width * 0.5f, Height * 0.5f);
    ...
    public World(int width, int height, int workerThreads)
    {
        ...
        _trainingPersistence = new TrainingPersistence(Path.Combine(AppContext.BaseDirectory, "training"));

        if (_trainingPersistence.TryLoadLatest(out var loadedBank))
        {
            _trainingBank = loadedBank;
            _lastPersistenceEvent = "loaded prey + predator training";
        }
    ...
    public void Reset()
    {
        ...
        _autosaveTimer = 0f;
        _nextId = 1;
        _preyRecoveryPending = false;
        _trainingDirty = false;
        Selection = new SelectionInfo((Agent?)null);
        OldestLivingPrey = null;
        OldestLivingPredator = null;
        Preys.Clear();
        Predators.Clear();
        Plants.Clear();
        Foods.Clear();
        ...
        if (HasTrainingSeeds())
        {
            PreyCulture.ApplyState(_trainingBank.PreyCulture);
            PredatorCulture.ApplyState(_trainingBank.PredatorCulture);
        }
        else
        {
            PreyCulture.Reset();
        ...
        SpawnPopulation(Species.Prey, PreyPopulationCount);
        SpawnPopulation(Species.Predator, PredatorPopulationCount);

        if (HasTrainingSeeds())
        {
            _lastPersistenceEvent = "reloaded prey + predator training";
        }
    }
    ...
    private static (Color Head, Color Mid, Color Tail) GetPreyPalette(Sex sex)
    {
        return sex == Sex.Male
            ? (Color.FromArgb(92, 212, 122), Color.FromArgb(70, 186, 102), Color.FromArgb(54, 152, 82))
            : (Color.FromArgb(156, 110, 238), Color.FromArgb(132, 90, 220), Color.FromArgb(108, 74, 194));
    }

    private static (Color Head, Color Mid, Color Tail) GetPredatorPalette(Sex sex)
    {
        return sex == Sex.Male
            ? (Color.FromArgb(255, 92, 76), Color.FromArgb(232, 74, 58), Color.FromArgb(194, 58, 44))
            : (Color.FromArgb(255, 74, 168), Color.FromArgb(232, 56, 146), Color.FromArgb(196, 44, 120));
    }
    ...
    public void BuildDrawLists(List<DrawItem> shadows, List<DrawItem> bodies, List<DrawItem> rings)
    {
        ...
        foreach (var prey in Preys)
        {
            ...
            var perp = heading.Perpendicular();
            var preyPalette = GetPreyPalette(prey.Sex);
            shadows.Add(new DrawItem(prey.Position.X, prey.Position.Y + (prey.Radius * 0.40f), prey.Radius * 1.45f, prey.Radius * 0.54f, 0f, 0f, 0f, prey.Position.Y, 0.28f, Color.Black, 1));
            for (var seg = 0; seg < 3; seg++)
            {
                var t = seg / 2f;
                var segPos = prey.Position - heading * (seg * prey.Radius * 0.55f);
                var segSize = prey.Radius * (1.0f - seg * 0.16f);
                var segElev = prey.Elevation * (1.0f - seg * 0.10f);
                var color = seg == 0 ? preyPalette.Head : (seg == 1 ? preyPalette.Mid : preyPalette.Tail);
                bodies.Add(new DrawItem(segPos.X, segPos.Y, segSize, segSize * (0.92f + 0.03f * t), segElev, heading.X, heading.Y, segPos.Y + segElev + seg * 0.02f, 1f, color, 0));
            }
            var eyeOffset = perp * (prey.Radius * 0.24f) + heading * (prey.Radius * 0.22f);
            bodies.Add(new DrawItem(prey.Position.X + eyeOffset.X, prey.Position.Y + eyeOffset.Y, prey.Radius * 0.18f, prey.Radius * 0.18f, prey.Elevation + 0.2f, heading.X, heading.Y, prey.Position.Y + prey.Elevation + 0.3f, 0.95f, Color.FromArgb(245, 250, 255), 0));
            bodies.Add(new DrawItem(prey.Position.X - eyeOffset.X + heading.X * (prey.Radius * 0.44f), prey.Position.Y - eyeOffset.Y + heading.Y * (prey.Radius * 0.44f), prey.Radius * 0.18f, prey.Radius * 0.18f, prey.Elevation + 0.2f, heading.X, heading.Y, prey.Position.Y + prey.Elevation + 0.31f, 0.95f, Color.FromArgb(245, 250, 255), 0));
            bodies.Add(new DrawItem(prey.Position.X + eyeOffset.X + heading.X * (prey.Radius * 0.08f), prey.Position.Y + eyeOffset.Y + heading.Y * (prey.Radius * 0.08f), prey.Radius * 0.08f, prey.Radius * 0.08f, prey.Elevation + 0.22f, heading.X, heading.Y, prey.Position.Y + prey.Elevation + 0.32f, 1f, Color.FromArgb(12, 20, 28), 0));
            bodies.Add(new DrawItem(prey.Position.X - eyeOffset.X + heading.X * (prey.Radius * 0.52f), prey.Position.Y - eyeOffset.Y + heading.Y * (prey.Radius * 0.52f), prey.Radius * 0.08f, prey.Radius * 0.08f, prey.Elevation + 0.22f, heading.X, heading.Y, prey.Position.Y + prey.Elevation + 0.33f, 1f, Color.FromArgb(12, 20, 28), 0));
            if (prey.IsElder)
            {
                rings.Add(new DrawItem(prey.Position.X, prey.Position.Y, prey.Radius + 10f + prey.ReinvestmentStreak, prey.Radius + 10f + prey.ReinvestmentStreak, 0f, 0f, 0f, prey.Position.Y + prey.Elevation + 2f, 0.55f, Color.FromArgb(255, 240, 120), 2));
            }
        }
        ...
        foreach (var predator in Predators)
        {
            ...
            var perp = heading.Perpendicular();
            var predatorPalette = GetPredatorPalette(predator.Sex);
            shadows.Add(new DrawItem(predator.Position.X, predator.Position.Y + (predator.Radius * 0.42f), predator.Radius * 1.55f, predator.Radius * 0.58f, 0f, 0f, 0f, predator.Position.Y, 0.34f, Color.Black, 1));
            for (var seg = 0; seg < 3; seg++)
            {
                var segPos = predator.Position - heading * (seg * predator.Radius * 0.62f);
                var segSize = predator.Radius * (1.04f - seg * 0.18f);
                var segElev = predator.Elevation * (1.0f - seg * 0.10f);
                var color = seg == 0 ? predatorPalette.Head : (seg == 1 ? predatorPalette.Mid : predatorPalette.Tail);
                bodies.Add(new DrawItem(segPos.X, segPos.Y, segSize, segSize * 0.92f, segElev, heading.X, heading.Y, segPos.Y + segElev + seg * 0.02f, 1f, color, 0));
            }
            var eyeOffset = perp * (predator.Radius * 0.22f) + heading * (predator.Radius * 0.26f);
            bodies.Add(new DrawItem(predator.Position.X + eyeOffset.X, predator.Position.Y + eyeOffset.Y, predator.Radius * 0.16f, predator.Radius * 0.16f, predator.Elevation + 0.24f, heading.X, heading.Y, predator.Position.Y + predator.Elevation + 0.35f, 0.98f, Color.FromArgb(255, 244, 226), 0));
            bodies.Add(new DrawItem(predator.Position.X - eyeOffset.X + heading.X * (predator.Radius * 0.52f), predator.Position.Y - eyeOffset.Y + heading.Y * (predator.Radius * 0.52f), predator.Radius * 0.16f, predator.Radius * 0.16f, predator.Elevation + 0.24f, heading.X, heading.Y, predator.Position.Y + predator.Elevation + 0.36f, 0.98f, Color.FromArgb(255, 244, 226), 0));
            bodies.Add(new DrawItem(predator.Position.X + eyeOffset.X + heading.X * (predator.Radius * 0.12f), predator.Position.Y + eyeOffset.Y + heading.Y * (predator.Radius * 0.12f), predator.Radius * 0.07f, predator.Radius * 0.07f, predator.Elevation + 0.26f, heading.X, heading.Y, predator.Position.Y + predator.Elevation + 0.37f, 1f, Color.FromArgb(34, 12, 8), 0));
            bodies.Add(new DrawItem(predator.Position.X - eyeOffset.X + heading.X * (predator.Radius * 0.62f), predator.Position.Y - eyeOffset.Y + heading.Y * (predator.Radius * 0.62f), predator.Radius * 0.07f, predator.Radius * 0.07f, predator.Elevation + 0.26f, heading.X, heading.Y, predator.Position.Y + predator.Elevation + 0.38f, 1f, Color.FromArgb(34, 12, 8), 0));
            if (predator.IsElder)
            {
                rings.Add(new DrawItem(predator.Position.X, predator.Position.Y, predator.Radius + 10f + predator.ReinvestmentStreak, predator.Radius + 10f + predator.ReinvestmentStreak, 0f, 0f, 0f, predator.Position.Y + predator.Elevation + 2f, 0.55f, Color.FromArgb(255, 200, 90), 2));
            }
        }
    ...
    private void Step(float dt)
    {
        ...
        {
            _autosaveTimer = 0f;
            StageTrainingSnapshot("autosave-buffer");
            FlushTrainingToDisk();
        }

        if (Preys.Count == 0 && Predators.Count == 0)
        {
            SaveTrainingNow("extinction");
            _trainingBank.RebirthCount++;
            Reset();
            _lastPersistenceEvent = $"extinction rebirth #{_trainingBank.RebirthCount}";
            return;
        }

        UpdatePendingRecovery(dt);

        if (Preys.Count == 0 && Predators.Count > 0)
        {
            if (!_preyRecoveryPending)
            {
                BeginPreyRecoveryCycle();
            }
        }

        if (Predators.Count == 0 && Preys.Count > 0)
        {
            RecoverCollapsedSpecies(
                Species.Predator,
                Math.Max(8, PredatorPopulationCount),
                "predator-collapse",
                $"predator collapse recovery #{_trainingBank.RebirthCount + 1}");
            return;
        }
    ...
    private void UpdatePrey(Agent agent, int index, float dt)
    {
        ...
        var nearestPlant = FindNearestPlant(agent.Position, agent.Traits.VisionRange * 0.85f, out var shelterPlant, out var shelterDistance, out var shelterDirection);
        var readyForMate = CanSeekMate(agent, Species.Prey);
        Agent? mate = null;
        var mateDistance = 0f;
        var mateDirection = Vec2.Zero;
        var nearestMate = readyForMate && FindNearestMate(agent, Preys, Math.Max(agent.Traits.SocialRange * 1.75f, agent.Traits.VisionRange * 0.90f), out mate, out mateDistance, out mateDirection);
        ...
        if (nearestMate && !nearestPredator) steer += mateDirection * (0.54f + Math.Max(0f, agent.Outputs[7]) * 0.18f);
        ...
        if (nearestPlant && nearestPredator)
        {
            var hideWeight = 0.34f + (agent.HideAffinity * 0.46f) + Math.Max(0f, agent.Outputs[6]) * 0.20f;
            steer += shelterDirection * hideWeight;
            if (shelterDistance <= Math.Max(8f, shelterPlant!.Radius * 1.1f))
            {
                steer -= predatorDirection * (0.18f + agent.HideAffinity * 0.20f);
            }
        }
        ...
        agent.LastStatus = nearestPredator ? (nearestPlant ? "hiding from danger" : "evading danger") : nearestMate ? $"seeking {mate!.Sex.ToString().ToLower()} mate" : nearestFood ? "foraging" : nearestWater ? "seeking water" : "wandering";
        agent.WanderPhase += dt * (0.35f + (agent.CuriosityBias * 0.20f));
        agent.TargetFoodIndex = foodIndex;
        agent.TargetWaterIndex = waterIndex;
        agent.MateTargetId = nearestMate ? mate!.Id : -1;
    ...
    private void UpdatePredator(Agent agent, int index, float dt)
    {
        ...
        var nearestPlant = FindNearestPlant(agent.Position, agent.Traits.VisionRange * 0.70f, out var ambushPlant, out var ambushDistance, out var ambushDirection);
        var readyForMate = CanSeekMate(agent, Species.Predator);
        Agent? mate = null;
        var mateDistance = 0f;
        var mateDirection = Vec2.Zero;
        var nearestMate = readyForMate && FindNearestMate(agent, Predators, Math.Max(agent.Traits.SocialRange * 1.85f, agent.Traits.VisionRange * 0.82f), out mate, out mateDistance, out mateDirection);
        ...
        if (nearestMate && !nearestPrey) steer += mateDirection * (0.44f + Math.Max(0f, agent.Outputs[7]) * 0.14f);
        ...
        if (nearestPlant && nearestPrey && ambushDistance < preyDistance)
        {
            steer += ambushDirection * (0.16f + Math.Max(0f, agent.Outputs[6]) * 0.12f);
        }
        ...
        agent.LastStatus = nearestPrey ? (nearestPlant && ambushDistance < preyDistance ? "ambushing" : "hunting") : nearestMate ? $"seeking {mate!.Sex.ToString().ToLower()} mate" : nearestWater ? "heading to water" : nearestCarrion ? "scavenging" : "patrolling";
    ...
    private float GetLegacyPressure(Agent agent)
    {
        var ratio = agent.HiddenDeathAge <= 1f ? 1f : Math.Clamp(agent.Age / agent.HiddenDeathAge, 0f, 1.4f);
        var pressure = Math.Clamp((ratio - 0.52f) / 0.48f, 0f, 1f);
        if (agent.Energy >= agent.Traits.ReproduceEnergy * 1.60f) pressure += 0.12f;
        return Math.Clamp(pressure, 0f, 1f);
    }

    private float GetBirthOpportunityThreshold(Agent agent, Species species)
    {
        var baseThreshold = species == Species.Prey ? 0.98f : 1.00f;
        var pressure = GetLegacyPressure(agent);
        return Math.Max(0.92f, baseThreshold - (pressure * 0.14f));
    }

    private bool CanAttemptLegacyChoice(Agent agent, Species species)
    {
        if (agent.ReproductionCooldown > 0f) return false;
        if (agent.Age < (species == Species.Prey ? 14f : 18f)) return false;
        return agent.Energy >= agent.Traits.ReproduceEnergy * GetBirthOpportunityThreshold(agent, species);
    }

    private void ApplyTraitTradeIn(Agent agent)
    {
        agent.ReinvestmentStreak++;
        agent.DeferredOffspring = Math.Max(agent.DeferredOffspring, (agent.ReinvestmentStreak + 1) / 2);
        agent.Energy = Math.Max(10f, agent.Energy - (agent.Traits.ReproduceEnergy * 0.10f));
        agent.Hydration = Math.Min(100f, agent.Hydration + 4f);
        agent.Age = Math.Max(0f, agent.Age - 2.2f);
        ...
        switch (pick)
        {
            case 0:
                agent.VisionTrades++;
                agent.Traits.VisionRange = Math.Clamp(agent.Traits.VisionRange + 5f, 50f, 440f);
                agent.LastStatus = "traded birth for vision";
                break;
            case 1:
                agent.SpeedTrades++;
                agent.Traits.MaxSpeed = Math.Clamp(agent.Traits.MaxSpeed + 1.8f, 50f, 228f);
                agent.LastStatus = "traded birth for speed";
                break;
            case 2:
                agent.AccelerationTrades++;
                agent.Traits.Acceleration = Math.Clamp(agent.Traits.Acceleration + 2.0f, 30f, 228f);
                agent.LastStatus = "traded birth for burst";
                break;
            case 3:
                agent.EfficiencyTrades++;
                agent.Traits.Metabolism = Math.Clamp(agent.Traits.Metabolism - 0.012f, 0.10f, 2.1f);
                agent.Traits.WaterUse = Math.Clamp(agent.Traits.WaterUse - 0.008f, 0.06f, 1.2f);
                agent.LastStatus = "traded birth for efficiency";
                break;
            default:
                agent.AwarenessTrades++;
                agent.Traits.MemoryStrength = Math.Clamp(agent.Traits.MemoryStrength + 0.015f, 0.05f, 1.40f);
                agent.Traits.Courage = Math.Clamp(agent.Traits.Courage + 0.008f, 0.05f, 1.25f);
                agent.Traits.Curiosity = Math.Clamp(agent.Traits.Curiosity + 0.008f, 0.05f, 1.25f);
                agent.HideAffinity = Math.Clamp(agent.HideAffinity + 0.015f, 0.05f, 1.30f);
                agent.LastStatus = "traded birth for awareness";
                break;
        }

        agent.PeakReinvestmentStreak = Math.Max(agent.PeakReinvestmentStreak, agent.ReinvestmentStreak);
        agent.Brain.Reinforce(agent.Outputs, 0.10f);
        agent.ReproductionCooldown = 2.2f + (float)_rng.NextDouble() * 1.6f;
    }

    private bool ShouldInvestInSelf(Agent agent, Species species)
    {
        var ownCount = species == Species.Prey ? Preys.Count : Predators.Count;
        var otherCount = species == Species.Prey ? Predators.Count : Preys.Count;

        var ownSafeFloor = species == Species.Prey ? 26f : 8f;
        var otherSafeFloor = species == Species.Prey ? 6f : 20f;

        var ownScarcity = 1f - Math.Clamp(ownCount / ownSafeFloor, 0f, 1f);
        var otherScarcity = 1f - Math.Clamp(otherCount / otherSafeFloor, 0f, 1f);
        var worldFragility = Math.Clamp((ownScarcity * 0.58f) + (otherScarcity * 0.42f), 0f, 1f);

        var balanceTarget = species == Species.Prey ? 7.5f : 0.16f;
        var actualBalance = ownCount / Math.Max(1f, otherCount);
        var balanceError = species == Species.Prey
            ? Math.Clamp((balanceTarget - actualBalance) / balanceTarget, -1f, 1f)
            : Math.Clamp((actualBalance - balanceTarget) / Math.Max(0.01f, balanceTarget), -1f, 1f);

        var energyReserve = Math.Clamp(agent.Energy / Math.Max(1f, agent.Traits.ReproduceEnergy * 1.65f), 0f, 1.6f);
        var lowReserve = 1f - Math.Clamp(energyReserve, 0f, 1f);
        var caution = species == Species.Prey ? (1f - agent.CourageBias) : (0.30f + agent.AggressionBias * 0.22f);
        var neuralVote = agent.Outputs.Length > 6 ? (agent.Outputs[6] * 0.5f + 0.5f) : 0.5f;
        var legacyPressure = GetLegacyPressure(agent);

        var survivalPressure = Math.Clamp((worldFragility * 0.55f) + (Math.Max(0f, balanceError) * 0.30f), 0f, 1f);
        var investScore = (energyReserve * 0.28f) + (caution * 0.16f) + (neuralVote * 0.15f) + (lowReserve * 0.10f);
        investScore += Math.Max(0f, -balanceError) * 0.18f;
        investScore -= survivalPressure * 0.72f;
        investScore -= legacyPressure * 0.95f;

        if (ownCount <= (species == Species.Prey ? 12 : 4))
        {
            investScore -= 0.55f;
        }

        if (otherCount <= (species == Species.Prey ? 3 : 10))
        {
            investScore -= species == Species.Predator ? 0.36f : 0.18f;
        }

        if (worldFragility >= 0.80f)
        {
            investScore -= 0.45f;
        }

        if (legacyPressure >= 0.64f && agent.DeferredOffspring > 0) return false;
        if (legacyPressure >= 0.80f) return false;
        return investScore >= 0.58f;
    }

    private void ApplyReinvestment(Agent agent)
    {
        ApplyTraitTradeIn(agent);
        agent.Radius = agent.Traits.Size;
    }

    private int DetermineBirthCount(Agent agent)
    {
        return Math.Max(1, Math.Max(agent.DeferredOffspring, (agent.ReinvestmentStreak + 1) / 2));
    }
    ...
    private void ResolveReproduction()
    {
        foreach (var prey in Preys)
        {
            if (prey.IsDead || prey.Sex != Sex.Female) continue;
            if (!CanAttemptLegacyChoice(prey, Species.Prey)) continue;

            if (ShouldInvestInSelf(prey, Species.Prey))
            {
                ApplyReinvestment(prey);
                prey.LastStatus = "holding line for survival";
                continue;
            }

            if (!TryFindMateForBirth(prey, Species.Prey, out var mate) || mate is null)
            {
                prey.LastStatus = "ready but no mate";
                continue;
            }

            var births = DetermineBirthCount(prey);
            var legacyBurst = GetLegacyPressure(prey) >= 0.68f || GetLegacyPressure(mate) >= 0.68f;
            prey.Energy *= legacyBurst ? 0.68f : 0.64f;
            mate.Energy *= legacyBurst ? 0.76f : 0.72f;
            prey.PeakReinvestmentStreak = Math.Max(prey.PeakReinvestmentStreak, prey.ReinvestmentStreak);
            mate.PeakReinvestmentStreak = Math.Max(mate.PeakReinvestmentStreak, mate.ReinvestmentStreak);
            prey.ReinvestmentStreak = 0;
            mate.ReinvestmentStreak = 0;
            prey.DeferredOffspring = 0;
            mate.DeferredOffspring = 0;
            prey.ReproductionCooldown = 7.2f + (float)(_rng.NextDouble() * 2.8);
            mate.ReproductionCooldown = 5.4f + (float)(_rng.NextDouble() * 2.4);
            for (var i = 0; i < births; i++)
            {
                lock (_spawnLock)
                {
                    _newPrey.Add(SpawnChild(prey, mate));
                }
            }
            prey.LastStatus = births > 1 ? $"mated legacy burst x{births}" : "mated and secured lineage";
            mate.LastStatus = $"mated with #{prey.Id}";
        }

        foreach (var predator in Predators)
        {
            if (predator.IsDead || predator.Sex != Sex.Female) continue;
            if (!CanAttemptLegacyChoice(predator, Species.Predator)) continue;

            if (ShouldInvestInSelf(predator, Species.Predator))
            {
                ApplyReinvestment(predator);
                predator.LastStatus = "holding line for survival";
                continue;
            }

            if (!TryFindMateForBirth(predator, Species.Predator, out var mate) || mate is null)
            {
                predator.LastStatus = "ready but no mate";
                continue;
            }

            var births = DetermineBirthCount(predator);
            var legacyBurst = GetLegacyPressure(predator) >= 0.68f || GetLegacyPressure(mate) >= 0.68f;
            predator.Energy *= legacyBurst ? 0.66f : 0.62f;
            mate.Energy *= legacyBurst ? 0.74f : 0.70f;
            predator.PeakReinvestmentStreak = Math.Max(predator.PeakReinvestmentStreak, predator.ReinvestmentStreak);
            mate.PeakReinvestmentStreak = Math.Max(mate.PeakReinvestmentStreak, mate.ReinvestmentStreak);
            predator.ReinvestmentStreak = 0;
            mate.ReinvestmentStreak = 0;
            predator.DeferredOffspring = 0;
            mate.DeferredOffspring = 0;
            predator.ReproductionCooldown = 8.4f + (float)(_rng.NextDouble() * 3.0);
            mate.ReproductionCooldown = 6.0f + (float)(_rng.NextDouble() * 2.6);
            for (var i = 0; i < births; i++)
            {
                lock (_spawnLock)
                {
                    _newPredators.Add(SpawnChild(predator, mate));
                }
            }
            predator.LastStatus = births > 1 ? $"mated legacy burst x{births}" : "mated and secured lineage";
            mate.LastStatus = $"mated with #{predator.Id}";
        }
    }
    ...
    private void UpdateElders()
    {
        ...
        foreach (var prey in Preys)
        {
            prey.IsElder = false;
            if (prey.IsDead) continue;
            if (OldestLivingPrey is null || prey.Age > OldestLivingPrey.Age)
            {
                OldestLivingPrey = prey;
            }
        }

        foreach (var predator in Predators)
        {
            predator.IsElder = false;
            if (predator.IsDead) continue;
            if (OldestLivingPredator is null || predator.Age > OldestLivingPredator.Age)
            {
                OldestLivingPredator = predator;
            }
        }

        if (OldestLivingPrey is not null) OldestLivingPrey.IsElder = true;
        if (OldestLivingPredator is not null) OldestLivingPredator.IsElder = true;
    }
    ...
    private Agent SpawnChild(Agent mother, Agent father)
    {
        var center = (mother.Position + father.Position) * 0.5f;
        var offset = Vec2.FromAngle((float)(_rng.NextDouble() * Math.PI * 2.0f)) * (MathF.Max(mother.Radius, father.Radius) + 10f + (float)(_rng.NextDouble() * 10.0));
        var traits = BlendTraits(mother.Traits, father.Traits, mother.Species).Mutated(_rng, mother.Species);
        var generation = Math.Max(mother.Generation, father.Generation) + 1;
        var child = new Agent
        {
            Id = _nextId++,
            Species = mother.Species,
            Sex = _rng.NextDouble() < 0.5 ? Sex.Male : Sex.Female,
            Position = ClampToWorld(center + offset, traits.Size + 2f),
            Velocity = Vec2.FromAngle((float)(_rng.NextDouble() * Math.PI * 2.0f)) * (10f + (float)(_rng.NextDouble() * 16.0)),
            Energy = traits.ReproduceEnergy * (mother.Species == Species.Prey ? 0.58f : 0.54f),
            Hydration = 72f + (float)(_rng.NextDouble() * 20f),
            Age = 0f,
            Generation = generation,
            Brain = Brain.Breed(mother.Brain, father.Brain, _rng, traits.MutationSigma),
            Traits = traits,
            ...
            CooperationBias = Math.Clamp(((mother.CooperationBias + father.CooperationBias) * 0.5f) + Signed(0.04f), 0f, 1f),
            CuriosityBias = Math.Clamp(((mother.CuriosityBias + father.CuriosityBias) * 0.5f) + Signed(0.04f), 0f, 1f),
            CourageBias = Math.Clamp(((mother.CourageBias + father.CourageBias) * 0.5f) + Signed(0.04f), 0f, 1f),
            AggressionBias = Math.Clamp(((mother.AggressionBias + father.AggressionBias) * 0.5f) + Signed(0.04f), 0f, 1f),
            ReinvestmentStreak = 0,
            PeakReinvestmentStreak = Math.Max(Math.Max(mother.PeakReinvestmentStreak, mother.ReinvestmentStreak), Math.Max(father.PeakReinvestmentStreak, father.ReinvestmentStreak)),
            DeferredOffspring = 0,
            VisionTrades = Math.Max(mother.VisionTrades, father.VisionTrades),
            SpeedTrades = Math.Max(mother.SpeedTrades, father.SpeedTrades),
            AccelerationTrades = Math.Max(mother.AccelerationTrades, father.AccelerationTrades),
            EfficiencyTrades = Math.Max(mother.EfficiencyTrades, father.EfficiencyTrades),
            AwarenessTrades = Math.Max(mother.AwarenessTrades, father.AwarenessTrades),
            HiddenDeathAge = RandomHiddenDeathAge(traits, generation),
            HideAffinity = Math.Clamp(((mother.HideAffinity + father.HideAffinity) * 0.5f) + Signed(0.03f), 0.05f, 1f),
            LastStatus = "newborn"
        };

        child.FoodMemory = Vec2.Lerp(mother.FoodMemory, father.FoodMemory, 0.5f);
        child.DangerMemory = Vec2.Lerp(mother.DangerMemory, father.DangerMemory, 0.5f);
        child.WaterMemory = Vec2.Lerp(mother.WaterMemory, father.WaterMemory, 0.5f);
        child.HuntMemory = Vec2.Lerp(mother.HuntMemory, father.HuntMemory, 0.5f);
        child.FoodMemoryStrength = MathF.Max(mother.FoodMemoryStrength, father.FoodMemoryStrength) * 0.36f;
        child.DangerMemoryStrength = MathF.Max(mother.DangerMemoryStrength, father.DangerMemoryStrength) * 0.36f;
        child.WaterMemoryStrength = MathF.Max(mother.WaterMemoryStrength, father.WaterMemoryStrength) * 0.36f;
        child.HuntMemoryStrength = MathF.Max(mother.HuntMemoryStrength, father.HuntMemoryStrength) * 0.36f;
        return child;
    }

    private AgentTraits BlendTraits(AgentTraits a, AgentTraits b, Species species)
    {
        var t = new AgentTraits
        {
            MaxSpeed = (a.MaxSpeed + b.MaxSpeed) * 0.5f,
            Acceleration = (a.Acceleration + b.Acceleration) * 0.5f,
            VisionRange = (a.VisionRange + b.VisionRange) * 0.5f,
            SocialRange = (a.SocialRange + b.SocialRange) * 0.5f,
            Metabolism = (a.Metabolism + b.Metabolism) * 0.5f,
            WaterUse = (a.WaterUse + b.WaterUse) * 0.5f,
            ReproduceEnergy = (a.ReproduceEnergy + b.ReproduceEnergy) * 0.5f,
            MaxAge = (a.MaxAge + b.MaxAge) * 0.5f,
            Size = (a.Size + b.Size) * 0.5f,
            MutationSigma = (a.MutationSigma + b.MutationSigma) * 0.5f,
            Communication = (a.Communication + b.Communication) * 0.5f,
            MemoryStrength = (a.MemoryStrength + b.MemoryStrength) * 0.5f,
            Courage = (a.Courage + b.Courage) * 0.5f,
            Curiosity = (a.Curiosity + b.Curiosity) * 0.5f,
            Cooperation = (a.Cooperation + b.Cooperation) * 0.5f,
            Lift = (a.Lift + b.Lift) * 0.5f
        };

        if (species == Species.Predator)
        {
            t.Courage = Math.Clamp(t.Courage + 0.03f, 0.05f, 1f);
        }
        else
        {
            t.ReproduceEnergy = Math.Max(42f, t.ReproduceEnergy - 1.5f);
        }
    ...
    private static bool IsMature(Agent agent, Species species)
    {
        return agent.Age >= (species == Species.Prey ? 14f : 18f);
    }

    private static bool CanSeekMate(Agent agent, Species species)
    {
        if (agent.ReproductionCooldown > 0f) return false;
        if (!IsMature(agent, species)) return false;
        return agent.Energy >= agent.Traits.ReproduceEnergy * 0.82f;
    }

    private static bool FindNearestMate(Agent seeker, List<Agent> population, float range, out Agent? bestMate, out float bestDistance, out Vec2 bestDirection)
    {
        bestMate = null;
        bestDistance = float.MaxValue;
        bestDirection = Vec2.Zero;
        ...
        for (var i = 0; i < population.Count; i++)
        {
            ...
            if (ReferenceEquals(candidate, seeker) || candidate.IsDead) continue;
            if (candidate.Sex == seeker.Sex) continue;
            if (candidate.ReproductionCooldown > 0f) continue;
            var delta = candidate.Position - seeker.Position;
            var distance = delta.Length;
        ...
        return bestMate is not null;
    }

    private bool TryFindMateForBirth(Agent agent, Species species, out Agent? mate)
    {
        var population = species == Species.Prey ? Preys : Predators;
        var range = Math.Max(agent.Traits.SocialRange * 0.95f, agent.Radius * 3.5f);
        if (!FindNearestMate(agent, population, range, out mate, out var distance, out _)) return false;
        if (mate is null) return false;
        if (!CanAttemptLegacyChoice(mate, species)) return false;
        if (!IsMature(mate, species)) return false;
        return distance <= range;
    }

    private bool FindNearestPlant(Vec2 position, float vision, out Plant? bestPlant, out float bestDistance, out Vec2 bestDirection)
    {
        bestPlant = null;
        bestDistance = float.MaxValue;
        bestDirection = Vec2.Zero;
        ...
        {
            var plant = Plants[i];
            var delta = plant.Position - position;
            var distance = Math.Max(0f, delta.Length - plant.Radius);
            if (distance < vision && distance < bestDistance)
            {
                bestDistance = distance;
                bestPlant = plant;
                var len = delta.Length;
                bestDirection = len <= 0.0001f ? Vec2.Zero : delta / len;
            }
        }

        return bestPlant is not null;
    }
    ...
    private Agent CreateAgent(Species species, Vec2 position, int generation)
    {
        ...
        return new Agent
        {
            ...
            Energy = traits.ReproduceEnergy * (species == Species.Prey ? 0.74f : 0.68f),
            ...
            CourageBias = species == Species.Predator ? Math.Clamp(traits.Courage + 0.08f, 0f, 1f) : traits.Courage,
            AggressionBias = species == Species.Predator ? Math.Clamp(traits.Courage + 0.26f, 0f, 1f) : Math.Clamp(traits.Courage * 0.22f, 0f, 1f),
            Sex = _rng.NextDouble() < 0.5 ? Sex.Male : Sex.Female,
            VisionTrades = 0,
            SpeedTrades = 0,
            AccelerationTrades = 0,
            EfficiencyTrades = 0,
            AwarenessTrades = 0,
            HiddenDeathAge = RandomHiddenDeathAge(traits, generation),
            HideAffinity = species == Species.Prey ? 0.55f : 0.18f,
            LastStatus = "settling"
        };
    }


    public void SaveTrainingNow(string trigger = "manual")
    {
        CaptureTrainingState(trigger);
        FlushTrainingToDisk();
    }

    public void StageTrainingSnapshot(string trigger = "staged")
    {
        CaptureTrainingState(trigger);
        _lastPersistenceEvent = $"staged {trigger} in RAM -> disk sync every {AutosaveIntervalSeconds / 60f:0} min";
    }

    private void CaptureTrainingState(string trigger)
    {
        UpdateTrainingBankFromCurrentWorld();
        _trainingBank.Trigger = trigger;
        _trainingBank.SavedUtc = DateTime.UtcNow;
        _trainingDirty = true;
    }

    private void FlushTrainingToDisk()
    {
        if (!_trainingDirty) return;

        _trainingBank.SaveIndex++;
        _trainingPersistence.Save(_trainingBank);
        _trainingDirty = false;
        _lastPersistenceEvent = $"saved {_trainingBank.Trigger} -> {Path.GetFileName(_trainingPersistence.PreyPath)} + {Path.GetFileName(_trainingPersistence.PredatorPath)}";
    }

    private bool HasTrainingSeeds()
    {
        return (_trainingBank.PreySeeds?.Count ?? 0) > 0 || (_trainingBank.PredatorSeeds?.Count ?? 0) > 0;
    }

    private void UpdateTrainingBankFromCurrentWorld()
    {
        if (Preys.Count > 0)
        {
            _trainingBank.PreySeeds = CaptureSeedPool(Preys, PreySeedPoolSize);
            _trainingBank.PreyCulture = PreyCulture.CaptureState();
        }

        if (Predators.Count > 0)
        {
            _trainingBank.PredatorSeeds = CaptureSeedPool(Predators, PredatorSeedPoolSize);
            _trainingBank.PredatorCulture = PredatorCulture.CaptureState();
        }
    }

    private List<AgentSeedState> CaptureSeedPool(List<Agent> agents, int maxCount)
    {
        var chosen = agents
            .Where(static agent => !agent.IsDead)
            .OrderByDescending(ScoreAgentForTraining)
            .Take(maxCount)
            .Select(CaptureAgentSeed)
            .ToList();

        return chosen;
    }

    private static float ScoreAgentForTraining(Agent agent)
    {
        return (agent.Energy * 1.15f) +
               (agent.Hydration * 0.55f) +
               (agent.Generation * 6f) +
               (agent.Traits.MaxAge * 0.03f) +
               (agent.Traits.VisionRange * 0.02f) +
               (agent.Traits.Communication * 6f) +
               (agent.Traits.MemoryStrength * 6f);
    }

    private static AgentSeedState CaptureAgentSeed(Agent agent)
    {
        return new AgentSeedState
        {
            Generation = agent.Generation,
            Sex = agent.Sex,
            Traits = agent.Traits.Clone(),
            Brain = agent.Brain.CaptureState(),
            CooperationBias = agent.CooperationBias,
            CuriosityBias = agent.CuriosityBias,
            CourageBias = agent.CourageBias,
            AggressionBias = agent.AggressionBias,
            FoodMemory = Vec2State.FromVec2(agent.FoodMemory),
            DangerMemory = Vec2State.FromVec2(agent.DangerMemory),
            WaterMemory = Vec2State.FromVec2(agent.WaterMemory),
            HuntMemory = Vec2State.FromVec2(agent.HuntMemory),
            FoodMemoryStrength = agent.FoodMemoryStrength,
            DangerMemoryStrength = agent.DangerMemoryStrength,
            WaterMemoryStrength = agent.WaterMemoryStrength,
            HuntMemoryStrength = agent.HuntMemoryStrength,
            ReinvestmentStreak = agent.ReinvestmentStreak,
            PeakReinvestmentStreak = agent.PeakReinvestmentStreak,
            DeferredOffspring = agent.DeferredOffspring,
            VisionTrades = agent.VisionTrades,
            SpeedTrades = agent.SpeedTrades,
            AccelerationTrades = agent.AccelerationTrades,
            EfficiencyTrades = agent.EfficiencyTrades,
            AwarenessTrades = agent.AwarenessTrades
        };
    }

    private void BeginPreyRecoveryCycle()
    {
        SaveTrainingNow("prey-collapse");
        _trainingBank.RebirthCount++;
        _preyRecoveryPending = true;
        _lastPersistenceEvent = "prey extinct: no prey return until every predator dies";
    }

    private void UpdatePendingRecovery(float dt)
    {
        _ = dt;

        if (!_preyRecoveryPending) return;
    ...
    private void RecoverCollapsedSpecies(Species species, int respawnCount, string trigger, string status)
    {
        SaveTrainingNow(trigger);
        _trainingBank.RebirthCount++;
        EnsureBaselineResources(species);
        SpawnPopulation(species, respawnCount);

        if (species == Species.Predator)
        {
            SeedEmergencyCarrion(Math.Max(10, respawnCount / 2));
        }
        else
        {
            SeedEmergencyFruit(Math.Max(24, respawnCount / 4));
        }

        _lastPersistenceEvent = status;
    }

    private void EnsureBaselineResources(Species recoveringSpecies)
    {
        while (Waters.Count < 5)
        {
            Waters.Add(new WaterSource
            {
                Position = RandomPosition(70f),
        ...
        var minimumPlants = recoveringSpecies == Species.Prey ? 120 : 90;
        while (Plants.Count < minimumPlants)
        {
            Plants.Add(CreatePlant(RandomPosition(18f)));
        }

        var minimumFood = recoveringSpecies == Species.Prey ? 220 : 120;
        while (Foods.Count < minimumFood)
        {
            Foods.Add(CreateFood(RandomPosition(12f), false));
        }
    }

    private void SeedEmergencyCarrion(int count)
    {
        for (var i = 0; i < count && Foods.Count < MaxFood; i++)
        {
            Foods.Add(CreateFood(RandomPosition(12f), true));
        }
    }

    private void SeedEmergencyFruit(int count)
    {
        for (var i = 0; i < count && Foods.Count < MaxFood; i++)
        {
            Foods.Add(CreateFood(RandomPosition(12f), false));
    ...
    private void SpawnPopulation(Species species, int count)
    {
        var seeds = species == Species.Prey ? _trainingBank.PreySeeds : _trainingBank.PredatorSeeds;
        var targetList = species == Species.Prey ? Preys : Predators;

        if (seeds is not null && seeds.Count > 0)
        {
            for (var i = 0; i < count; i++)
            {
                var seed = seeds[_rng.Next(seeds.Count)];
                targetList.Add(CreateAgentFromSeed(species, RandomPosition(10f), seed));
            }
            return;
        }
        ...
        for (var i = 0; i < count; i++)
        {
            targetList.Add(CreateAgent(species, RandomPosition(10f), 1));
        }
    }

    private Agent CreateAgentFromSeed(Species species, Vec2 position, AgentSeedState seed)
    {
        var baseTraits = seed.Traits?.Clone() ?? AgentTraits.CreateDefault(species);
        var traits = baseTraits.Mutated(_rng, species);
        var sourceBrain = Brain.FromState(seed.Brain, _rng);
        var preservedBrain = ((float)_rng.NextDouble() < 0.18f)
            ? sourceBrain
            : sourceBrain.MutatedClone(_rng, MathF.Max(0.01f, traits.MutationSigma * 0.55f));
        ...
        var agent = new Agent
        {
            ...
            Species = species,
            Position = position,
            Velocity = Vec2.FromAngle((float)(_rng.NextDouble() * Math.PI * 2.0)) * (10f + (float)(_rng.NextDouble() * 20.0)),
            Energy = traits.ReproduceEnergy * (species == Species.Prey ? 0.76f : 0.72f),
            Hydration = 72f + (float)(_rng.NextDouble() * 24.0),
            Age = 0f,
            Generation = Math.Max(1, seed.Generation + 1),
            Brain = preservedBrain,
            Traits = traits,
            ReproductionCooldown = 4f,
            BroadcastCooldown = 0.5f,
            Radius = traits.Size,
            Elevation = traits.Lift,
            WanderPhase = (float)(_rng.NextDouble() * Math.PI * 2.0),
            CooperationBias = Math.Clamp(seed.CooperationBias + Signed(0.04f), 0f, 1f),
            CuriosityBias = Math.Clamp(seed.CuriosityBias + Signed(0.04f), 0f, 1f),
            CourageBias = species == Species.Predator ? Math.Clamp(seed.CourageBias + 0.05f + Signed(0.03f), 0f, 1f) : Math.Clamp(seed.CourageBias + Signed(0.03f), 0f, 1f),
            AggressionBias = species == Species.Predator ? Math.Clamp(seed.AggressionBias + 0.05f + Signed(0.03f), 0f, 1f) : Math.Clamp(seed.AggressionBias * 0.30f + Signed(0.02f), 0f, 1f),
            Sex = _rng.NextDouble() < 0.5 ? Sex.Male : Sex.Female,
            ReinvestmentStreak = Math.Max(0, seed.ReinvestmentStreak),
            PeakReinvestmentStreak = Math.Max(0, seed.PeakReinvestmentStreak),
            DeferredOffspring = Math.Max(0, seed.DeferredOffspring),
            VisionTrades = Math.Max(0, seed.VisionTrades),
            SpeedTrades = Math.Max(0, seed.SpeedTrades),
            AccelerationTrades = Math.Max(0, seed.AccelerationTrades),
            EfficiencyTrades = Math.Max(0, seed.EfficiencyTrades),
            AwarenessTrades = Math.Max(0, seed.AwarenessTrades),
            HiddenDeathAge = RandomHiddenDeathAge(traits, Math.Max(1, seed.Generation + 1)),
            HideAffinity = species == Species.Prey ? 0.55f : 0.18f,
            LastStatus = "reborn from trained memory"
        };

        agent.FoodMemory = seed.FoodMemory?.ToVec2() ?? Vec2.Zero;
        agent.DangerMemory = seed.DangerMemory?.ToVec2() ?? Vec2.Zero;
        agent.WaterMemory = seed.WaterMemory?.ToVec2() ?? Vec2.Zero;
        agent.HuntMemory = seed.HuntMemory?.ToVec2() ?? Vec2.Zero;
        agent.FoodMemoryStrength = Math.Clamp(seed.FoodMemoryStrength * 0.60f, 0f, 1f);
        agent.DangerMemoryStrength = Math.Clamp(seed.DangerMemoryStrength * 0.60f, 0f, 1f);
        agent.WaterMemoryStrength = Math.Clamp(seed.WaterMemoryStrength * 0.60f, 0f, 1f);
        agent.HuntMemoryStrength = Math.Clamp(seed.HuntMemoryStrength * 0.60f, 0f, 1f);
        return agent;
    }

    private float RandomHiddenDeathAge(AgentTraits traits, int generation)
    {
        var lifeRoll = 0.78f + ((float)_rng.NextDouble() * 0.40f);
        var generationBonus = Math.Min(0.12f, generation * 0.004f);
        return Math.Max(50f, traits.MaxAge * (lifeRoll + generationBonus));
    }
