...
internal sealed class GpuRenderer : IDisposable
{
    ...
    public void Render(World world, int width, int height, Vec2 cameraCenter, float cameraZoom)
    {
        ...
        DrawInstances(_shadows, width, height, cameraCenter, cameraZoom);
        DrawInstances(_bodies, width, height, cameraCenter, cameraZoom);
        DrawInstances(_rings, width, height, cameraCenter, cameraZoom);
    ...
    private void DrawInstances(List<DrawItem> items, int width, int height, Vec2 cameraCenter, float cameraZoom)
    {
        ...
        GL.Uniform2(GL.GetUniformLocation(_circleProgram, "uCameraCenter"), cameraCenter.X, cameraCenter.Y);
        GL.Uniform1(GL.GetUniformLocation(_circleProgram, "uCameraZoom"), cameraZoom);
        GL.BindVertexArray(_quadVao);
        GL.BindBuffer(BufferTarget.ArrayBuffer, _instanceVbo);
    ...
    private const string CircleVertexShader = """
        ...
        void main()
        {
            vec2 worldPos = vec2(iRect.x + aCorner.x * iRect.z, iRect.y + aCorner.y * iRect.w - iMisc.x);
            vec2 pos = ((worldPos - uCameraCenter) * uCameraZoom) + (uViewport * 0.5);
