...
internal sealed class OverlayComposer
{
    ...
    public Bitmap Compose(int width, int height, World world, bool paused, bool showHelp, bool showUi, float simSpeed, float fps, bool fullscreen)
    {
        _buttonRects.Clear();
        var bitmap = new Bitmap(Math.Max(1, width), Math.Max(1, height), ImagingPixelFormat.Format32bppArgb);
        using var g = Graphics.FromImage(bitmap);
        ...
        if (!showUi)
        {
            var showRect = new RectangleF(width - 84f, height - 54f, 62f, 32f);
            _buttonRects[OverlayAction.ToggleUi] = showRect;
            DrawPanel(g, showRect, 12, Color.FromArgb(205, 20, 28, 38), Color.FromArgb(96, 255, 255, 255));
            using var format = new StringFormat { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center };
            g.DrawString("UI", smallFont, white, showRect, format);
            return bitmap;
        }
        ...
        g.DrawString("BioGenesis X", titleFont, white, top.X + 16, top.Y + 10);
        g.DrawString("GPU ecosystem sim + emergent survival learning", smallFont, faint, top.X + 18, top.Y + 36);
        g.DrawString("Neural agents ON   Learning ON   Population-aware survival instincts", smallFont, soft, top.X + 18, top.Y + 57);
        ...
        var controls = new[]
        {
            ...
            new ButtonInfo(OverlayAction.ToggleUi, "Hide UI"),
        ...
        var metaLines = BuildMetaLines(g, bodyFont, controlInnerWidth,
            ...
            $"Prey {world.Preys.Count}   Pred {world.Predators.Count}   Training {world.TrainingSlotLabel}   Rebirths {world.RebirthCount}");
        ...
        DrawStatRow(g, bodyFont, soft, x, y + 133, 116, "Training", world.TrainingSlotLabel);
        DrawStatRow(g, bodyFont, soft, x, y + 152, 116, "Rebirths", world.RebirthCount.ToString());
        DrawStatRow(g, bodyFont, soft, x, y + 171, 116, "Signals", world.Pulses.Count.ToString());
        y += 200;
        ...
        if (showHelp)
        {
            ...
            var helpText = string.Join(Environment.NewLine,
                ...
                "• Space pauses. R resets. H shows or hides this panel. Tab hides or reveals the whole UI.",
                "• Mouse wheel zooms. Right-drag pans. Click a creature to select it. F focuses the camera on the selection.",
                ...
                "• Adults weigh reproduction against self-reinvestment, but now also care about keeping the current world alive.",
                "• Offspring also inherit a partial carryover of learned output bias and remembered directions.",
                "• Species memory and live learning keep shared food, danger, water, hunt, and survival pressure knowledge alive.",
                "• Training stays hot in RAM and flushes to SSD every 10 minutes into Prey_Training.json and Predator_Training.json.",
                "• If prey are wiped out, the sim now waits through a famine window and only reintroduces a small prey line slowly, so over-hunting is punished.",
                "• When the full population crashes to zero, the sim saves again and rebirths a new world from the latest learned data.",
