...
internal sealed class MainForm : Form
{
    ...
    private readonly Button _autoRunButton = new();
    ...
    private void InitializeUi()
    {
        ...
        _autoRunButton.Text = "Auto Run: On";

        foreach (var button in new[] { _pauseButton, _clearButton, _clearObsButton, _fullscreenButton, _autoRunButton })
        {
            ...
            button.Height = 28;
            button.Width = button == _clearObsButton ? 112 : button == _autoRunButton ? 118 : 98;
        ...
        _autoRunButton.Click += (_, __) => ToggleAutoRun();
        ...
        _hintLabel.Text = "LMB: dye + force | RMB: obstacle | Shift+RMB: erase obstacle | Space: pause | A: auto run | F: fullscreen | H: shading";
        ...
        flow.Controls.Add(_autoRunButton);
    ...
    {
        _resolutionCombo.SelectedItem = "256 x 144";
        _threadsBox.Value = Math.Max(1, Math.Min(_threadsBox.Maximum, 48));
        _pressureBox.Value = 22;
    }
    ...
    private void DrawHud(Graphics g, Rectangle dest)
    {
        ...
        string line2 = $"Frame: {_lastFrameMs:F2} ms | FPS: {_fps:F1} | Pressure Iterations: {_sim.PressureIterations} | Auto Run: {(_autoRunEnabled ? "On" : "Off")}";
        string line3 = "Current path: CPU solver + optimized bitmap renderer. Auto Run paints flowing swirls so it starts pretty immediately.";
    ...
    private void ToggleAutoRun()
    {
        ...
        _autoRunButton.Text = _autoRunEnabled ? "Auto Run: On" : "Auto Run: Off";
    }

    private void AdvanceAutoRun(float dt)
    {
        _autoRunTime += dt;

        int radius = Math.Max(8, Math.Min(_sim.Width, _sim.Height) / 14);
        float cx = _sim.Width * 0.5f;
        float cy = _sim.Height * 0.5f;

        for (int i = 0; i < 3; i++)
        {
            float t = _autoRunTime * (0.55f + i * 0.17f) + i * 2.1f;
            float px = cx + MathF.Sin(t * (1.2f + i * 0.11f)) * (_sim.Width * (0.24f + i * 0.05f))
                         + MathF.Cos(t * (2.4f + i * 0.18f)) * (_sim.Width * 0.05f);
            float py = cy + MathF.Cos(t * (1.5f + i * 0.09f)) * (_sim.Height * (0.22f + i * 0.04f))
                         + MathF.Sin(t * (2.1f + i * 0.14f)) * (_sim.Height * 0.05f);

            float orbitDx = -(py - cy);
            float orbitDy = (px - cx);
            float orbitLen = MathF.Max(1f, MathF.Sqrt(orbitDx * orbitDx + orbitDy * orbitDy));
            orbitDx /= orbitLen;
            orbitDy /= orbitLen;

            float pulse = 0.8f + 0.35f * MathF.Sin(_autoRunTime * (1.4f + i * 0.2f) + i);
            float fx = orbitDx * (0.9f + i * 0.25f) * pulse;
            float fy = orbitDy * (0.9f + i * 0.25f) * pulse;

            ColorFromHue((_autoRunTime * 0.07f) + i / 3f, out float r, out float g, out float b);
            _sim.AddImpulseAndDye((int)px, (int)py, radius, fx, fy, r * 1.1f, g * 1.1f, b * 1.1f);
        }

        float centerPhase = _autoRunTime * 0.95f;
        float spiralPx = cx + MathF.Cos(centerPhase * 1.7f) * (_sim.Width * 0.10f);
        float spiralPy = cy + MathF.Sin(centerPhase * 1.9f) * (_sim.Height * 0.10f);
        ColorFromHue((_autoRunTime * 0.11f) + 0.45f, out float sr, out float sg, out float sb);
        _sim.AddImpulseAndDye((int)spiralPx, (int)spiralPy, Math.Max(6, radius - 2),
            MathF.Cos(centerPhase * 2.3f) * 1.15f,
            MathF.Sin(centerPhase * 2.0f) * 1.15f,
            sr * 0.9f, sg * 0.9f, sb * 0.9f);
    }
