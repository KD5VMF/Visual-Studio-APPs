...
public sealed class TrainingPersistence
{
    ...
    public string GetPathForSlot(int slot)
        => Path.Combine(_rootDirectory, $"Trained_Data-{slot:D2}.json");

    public bool TryLoadLatest(out TrainingBankState state)
    {
        state = new TrainingBankState();

        var files = Directory.GetFiles(_rootDirectory, "Trained_Data-*.json", SearchOption.TopDirectoryOnly);
        if (files.Length == 0) return false;

        TrainingBankState? best = null;
        foreach (var file in files)
        {
            try
            {
                var text = File.ReadAllText(file);
                var candidate = JsonSerializer.Deserialize<TrainingBankState>(text, _jsonOptions);
                if (candidate is null) continue;
                if (best is null ||
                    candidate.SaveIndex > best.SaveIndex ||
                    (candidate.SaveIndex == best.SaveIndex && candidate.SavedUtc > best.SavedUtc))
                {
                    best = candidate;
                }
            }
    ...
    {
        Directory.CreateDirectory(_rootDirectory);
        var path = GetPathForSlot(state.Slot);
...
public sealed class World
{
    ...
    private const int SaveSlotCount = 1001;
    ...
    public int CurrentTrainingSlot => _trainingBank.Slot;
    public long CurrentTrainingSaveIndex => _trainingBank.SaveIndex;
    public int RebirthCount => _trainingBank.RebirthCount;
    public string TrainingSlotLabel => _trainingBank.Slot < 0 ? "--" : _trainingBank.Slot.ToString("D2");
    ...
    public World(int width, int height, int workerThreads)
    {
        ...
        if (_trainingPersistence.TryLoadLatest(out var loadedBank))
        {
            ...
            _lastPersistenceEvent = $"loaded slot {TrainingSlotLabel}";
        }
    ...
    public void Reset()
    {
        ...
        if (HasTrainingSeeds())
        {
            _lastPersistenceEvent = $"reloaded slot {TrainingSlotLabel}";
        }
    }
    ...
    private void Step(float dt)
    {
        ...
        if (Preys.Count == 0)
        {
            RecoverCollapsedSpecies(
                ...
                PreyPopulationCount,
                "prey-collapse",
                $"prey collapse recovery #{_trainingBank.RebirthCount + 1}");
            return;
        }
    ...
    public void SaveTrainingNow(string trigger = "manual")
    {
        ...
        _trainingBank.Slot = (_trainingBank.Slot + 1 + SaveSlotCount) % SaveSlotCount;
        _trainingBank.Trigger = trigger;
        _trainingBank.SavedUtc = DateTime.UtcNow;
        _trainingPersistence.Save(_trainingBank);
        _lastPersistenceEvent = $"saved {trigger} -> {Path.GetFileName(_trainingPersistence.GetPathForSlot(_trainingBank.Slot))}";
    }
