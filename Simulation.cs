...
public sealed class World
{
    ...
    private const float AutosaveIntervalSeconds = 24f;
    ...
    public void SaveTrainingNow(string trigger = "manual")
    {
        ...
        _lastPersistenceEvent = $"saved {trigger} -> {Path.GetFileName(_trainingPersistence.PreyPath)} + {Path.GetFileName(_trainingPersistence.PredatorPath)}";
    }
    ...
    private Agent CreateAgentFromSeed(Species species, Vec2 position, AgentSeedState seed)
    {
        ...
        var agent = new Agent
        {
            ...
            Energy = traits.ReproduceEnergy * (species == Species.Prey ? 0.70f : 0.66f),
            Hydration = 72f + (float)(_rng.NextDouble() * 24.0),
            Age = 0f,
            ...
            CourageBias = Math.Clamp(seed.CourageBias + Signed(0.04f), 0f, 1f),
            AggressionBias = Math.Clamp(seed.AggressionBias + Signed(0.04f), 0f, 1f),
